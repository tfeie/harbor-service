...
public class UserJoinGroupConfirmListener implements InitializingBean {

	private static final Logger LOG = LoggerFactory.getLogger(UserJoinGroupConfirmListener.class);
	...
	public static void runThread(final IUserInterfactionSV userInterfactionSV) {
		...
		try {
			...
			LOG.error("活动确认消息工作线程启动失败", e);
		}
	}

	public static void WorkerFunc(int workerId, IUserInterfactionSV userInterfactionSV) {
		String queueName = GlobalSettings.getGroupJoinConfirmQueueName();
